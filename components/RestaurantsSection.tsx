  ...
  return (
    <section id="restaurants" className="mx-auto max-w-[1400px] px-6 py-24 md:px-10 md:py-32">
      <Reveal className="max-w-[46ch]">
        ...
        <h2 className="font-serif text-[34px] leading-tight md:text-[44px]">{restaurants.heading}</h2>
        <p className="mt-5 text-[18px] leading-snug text-muted">{restaurants.intro}</p>
      </Reveal>
      <div className="mt-14 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
        {restaurants.cards.map((card) => (
          <Reveal key={card.title}><FeatureCard card={card} variant="video" /></Reveal>
        ))}
      </div>
